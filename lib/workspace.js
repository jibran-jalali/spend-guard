...
export function createWorkspaceReports(workspace) {
  ...
  for (const payment of workspace.payments.filter((entry) => entry.status !== "projected")) {
    const key = format(startOfMonth(parseISO(payment.dueDate)), "MMM");
    monthlyTrendMap.set(key, (monthlyTrendMap.get(key) || 0) + payment.amount);
  }
